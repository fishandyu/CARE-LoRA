...
class LoraConfig(PeftConfig):
    ...
    # ===== CARE-LoRA variant =====
    use_care_lora: bool = field(
        default=False,
        metadata={
            "help": (
                "Use CARE-LoRA variant of LoRA: forward saves Z = X A^T and additionally uses the original X to compute the "
                "data-dependent best reconstruction M* = (Z^T Z + λ I)^{-1} Z^T X (Tikhonov). "
                "Then approx grad(A) is formed via grad_A ≈ (Z^T dZ)^T M*. Both A and B are updated by the standard optimizer."
            )
        },
    )

    care_lora_pinv_lambda: float = field(
        default=1e-6,
        metadata={
            "help": (
                "Tikhonov regularization coefficient λ used by CARE-LoRA for (Z^T Z + λ I)^{-1}. "
                "Larger λ improves numerical stability when Z is rank-deficient."
            )
        },
    )

    # ===== LoRAct baseline =====
    use_loract: bool = field(
        default=False,
        metadata={
            "help": (
                "Use LoRAct-style low-rank activation compression on the same LoRA Linear inputs selected by CARE-LoRA. "
                "Forward is standard LoRA; backward reconstructs X from a sampled QB decomposition to compute "
                "LoRA A/B gradients. Nonlinear activations are not compressed in this repo's LoRAct baseline."
            )
        },
    )
    loract_rank: int = field(
        default=64,
        metadata={
            "help": (
                "LoRAct sampled QB rank k. The effective rank is min(k, flattened_batch_tokens, hidden_size). "
                "This implementation uses the subsampled test matrix and exactly one QR decomposition."
            )
        },
    )
    ...
    use_lorafa: bool = field(
        ...
        metadata={
            "help": (
                "Use LoRA-FA variant: freeze A after initialization and train only B. "
                "No manual A update is used in this mode."
            )
        },
    )
    ...
    def __post_init__(self):
        ...
        if int(getattr(self, "loract_rank", 64)) < 1:
            raise ValueError("loract_rank must be >= 1.")

        if getattr(self, 'use_care_lora', False) and getattr(self, 'use_dora', False):
            raise ValueError('use_care_lora=True is incompatible with use_dora=True.')
        if getattr(self, 'use_lorafa', False) and getattr(self, 'use_dora', False):
            raise ValueError('use_lorafa=True is incompatible with use_dora=True.')
        if getattr(self, 'use_care_lora', False) and getattr(self, 'use_lorafa', False):
            raise ValueError('use_care_lora=True is incompatible with use_lorafa=True.')
        if getattr(self, 'use_loract', False) and getattr(self, 'use_dora', False):
            raise ValueError('use_loract=True is incompatible with use_dora=True.')
        if getattr(self, 'use_loract', False) and getattr(self, 'use_lorafa', False):
            raise ValueError('use_loract=True is incompatible with use_lorafa=True.')
        if getattr(self, 'use_loract', False) and getattr(self, 'use_care_lora', False):
            raise ValueError('use_loract=True is incompatible with use_care_lora=True.')
        if getattr(self, 'care_lora_pinv_lambda', 0.0) is not None and float(getattr(self, 'care_lora_pinv_lambda', 0.0)) < 0.0:
            raise ValueError('care_lora_pinv_lambda must be >= 0.')
        self.target_modules = (
